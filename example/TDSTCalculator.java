...
public class TDSTCalculator {

    public double getTDSD(File directory) throws IOException {
        ArrayList<Long> list = new ArrayList<>(Arrays.asList(0L, 0L));
        getTDSTFromTestFiles(directory, list);
        return (double) list.get(0) / list.get(1);
    }

    private void getTDSTFromTestFiles(File directory, ArrayList<Long> valuesList) throws IOException {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                getTDSTFromTestFiles(file, valuesList);
            }
    ...
    private long[] analyzeFile(File file) throws IOException {
        ...
        long methodsWithJavadoc = methods.stream().filter(m -> m.getJavadoc().isPresent()).count();
        long totalMethods = methods.size();
        return new long[]{methodsWithJavadoc, totalMethods};
    }
}
